...
@section('content')
    ...
    <section class="products_section">
        <div class="container">
            <div class="row bord_bottom">
                ...
                <div class="col-md-9 col-sm-8">
                    <div class="products_block_all">
                        <span>{{$meta->getMeta('search_total')}}
                            <strong class="count_filtering_products"> {{$count}} </strong> {{$meta->getMeta('search_found')}}</span>
                    </div>
            ...
            <div class="products_block_all">
                <span>{{$meta->getMeta('search_total')}} <strong
                            class="count_filtering_products"> {{$count}}</strong> {{$meta->getMeta('search_found')}}</span>
            </div>
        </div>
    </section>
@endsection
...
@section('js')
    ...
    <?php $categories = (isset($actual_category)) ? $actual_category : null; ?>
    <script>
        (function ($) {
            ...
            $("form.filtre input").on("change", function () { // only checkbox
                ...
                $('.show_all_products').html("Loading ...");
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{route('filter_product',['category'=>$categories])}}',
                    data: form.serialize(),
                    headers: {
